...
**个人微信接入教程的完整步骤是什么？**
> 7 步闭环：① 确认 CLI 已跑顺 ② 运行 `hermes gateway setup` 选 Weixin ③ 扫终端二维码 ④ 手机确认登录 ⑤ 确认凭据自动保存 ⑥ 写入 `WEIXIN_ACCOUNT_ID` 和私聊策略 ⑦ 启动 gateway 做第一条私聊测试。

**个人微信和企业微信接入有什么区别？**
> 个人微信（Weixin）走 iLink Bot API + QR 扫码登录 + HTTP 长轮询，适合个人移动端使用。企业微信（WeCom）走 AI Bot WebSocket 长连接 + 后台创建机器人 + Bot ID/Secret 凭据，适合团队协作场景。两者是完全不同的 adapter。

**个人微信接入后默认先开私聊还是群聊？**
> 默认先开私聊（DM）。官方文档中群聊策略默认更保守。建议先把私聊跑通排错成本最低，再根据需要配置 `WEIXIN_GROUP_POLICY` 和群白名单。

## 🚀 个人微信接入主线图

![个人微信接入主线图](./assets/weixin-entry-structure-v2.webp)
...
因为官方文档里的默认群策略就更保守，先把私聊跑通，排错成本最低。
...
- 接入前先用[命令行（CLI）](/docs/china/entry/cli)确认 Agent 能稳定响应。
- 想把同一能力暴露给网页客户端：看[API 服务与 Open WebUI](/docs/china/entry/api-service-open-webui)。
- 团队场景优先比较[企业微信 AI Bot](/docs/china/entry/wecom-ai-bot)。
- 主要使用 Apple 设备、希望改用原生信息入口：比较 [iMessage 接入（Photon Spectrum）](/docs/china/entry/imessage)；它是社区方案，不与官方 Weixin adapter 混写。
