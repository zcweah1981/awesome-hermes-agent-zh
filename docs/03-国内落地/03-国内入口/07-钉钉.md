...
> 💡 **速答**：Hermes Agent 接入钉钉走官方推荐的 Stream 模式——在钉钉开发者后台创建企业内部应用 → 开启机器人能力并选 Stream 模式 → 拿到 Client ID / Client Secret → 填入 Hermes 的 `DINGTALK_CLIENT_ID` 和 `DINGTALK_CLIENT_SECRET` → 启动 Gateway。不需要公网回调地址。
...
## 🖼️ 官方截图片段：机器人配置表单中的 Stream 模式

![钉钉官方机器人配置与 Stream 模式截图](./assets/dingtalk-robot-stream-official.webp)
...
- 这张图更适合证明“机器人配置页里确实存在 Stream 模式设置”
- 它不是“进入钉钉开发者后台入口”的截图
- 这页真正可执行的主线，仍然以你上面看到的中文步骤为准
...
- 如果团队使用企业微信，直接对比[企业微信 AI Bot](/docs/china/entry/wecom-ai-bot)。
