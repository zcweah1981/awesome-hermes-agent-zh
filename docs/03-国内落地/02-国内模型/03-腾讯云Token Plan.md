...
![腾讯云 Token Plan 接入流程示意图](./assets/tencent-tokenplan-hero-v3.png)
...
1. **订阅套餐**  
   先选 Lite / Standard / Pro / Max，按使用强度决定起点。

2. **生成 API 密钥**  
   在腾讯云控制台里一键创建 API 密钥。

3. **复制并保存**  
   把 API 密钥复制出来，保存到本地和你的配置记录里。

4. **配置编程工具**  
   填地址、模型、密钥，把 CodeBuddy、OpenClaw、OpenCode 等工具接起来。

如果你只想记住一句话：先把订阅和密钥跑通，再去细化工具和工作流。
...
- 第一步：在 TokenHub > Token Plan 页面点击“生成密钥”
- 第二步：密钥生成成功后点击“复制”，拿到套餐专属 API Key（格式类似 `sk-tp-xxx`）
