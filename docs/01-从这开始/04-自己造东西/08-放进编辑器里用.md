...
![08 放进编辑器里用：通过 ACP-compatible 编辑器连接 Hermes ACP server，让 Hermes 围绕当前项目、文件、终端和补丁进入 IDE 工作流](../../assets/rm2-5-acp-ide-01-editor-workflow-map-v2.webp)

---

## 🎯 先判断：你是不是已经进入“编辑器工作台”阶段

![判断是否已经进入编辑器工作台阶段：主界面转到编辑器、想围绕项目协作、不想来回切 CLI、已有 Hermes 配置，并理解仍主要靠 CLI 时可以后看](../../assets/rm2-5-acp-ide-02-editor-stage-check.webp)
...
![把 Hermes 接进编辑器的最短接法：确认 ACP-compatible 编辑器路线、安装 ACP extra、启动 Hermes ACP server、在编辑器里注册 Hermes，并复用原配置与凭据](../../assets/rm2-5-acp-ide-03-shortest-setup-path.webp)
...
![ACP 编辑器集成第一次失败时的排查顺序：确认 ACP extra 安装、编辑器兼容、hermes acp 可单独启动、原配置凭据可用、工作区上下文正确](../../assets/rm2-5-acp-ide-04-first-failure-checklist.webp)
...
- [把 Hermes 接进外部系统](/docs/start/build/mcp-and-plugins)
