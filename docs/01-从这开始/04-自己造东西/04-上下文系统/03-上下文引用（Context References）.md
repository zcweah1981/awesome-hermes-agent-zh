...
- 长期规则约束，用 [context files](./02-上下文文件（Context Files）.md)
...
- 长期规则仍然放在 [context files](./02-上下文文件（Context Files）.md)，没有混到 `@` 引用里
...
完成后进入：
- [把 Hermes 暴露成后端服务（API Server）](../05-把 Hermes 暴露成后端服务（API Server）.md)
