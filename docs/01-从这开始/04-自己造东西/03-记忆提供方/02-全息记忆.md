...
- 你现在的问题其实是助手角色混乱，更应该先看 [Profiles](../02-多助手.md)
...
- 回到 [外部记忆系统总览](./01-总览.md)，确认它在整体路线里的位置
- 回到 [持久记忆](../../03-玩出花样/03-持久记忆.md)，重新区分哪些信息该留在 `USER.md` / `MEMORY.md`，哪些才值得交给外部 provider

如果你发现自己真正要解决的是多助手拆分，而不是先跑通本地外部记忆，则应该回到 [Memory Providers](./01-总览.md) 这一层，重新判断后续路径。
