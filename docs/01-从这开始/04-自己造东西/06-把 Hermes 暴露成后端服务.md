...
如果你现在其实只是想找一个现成网页界面先把 Hermes 用起来，而不是自己接后端，先回到[03-国内落地 / 03-国内入口 / 02-网页控制台（Dashboard）](../../03-国内落地/03-国内入口/02-网页控制台（Dashboard）.md)。
...
如果你这里对[CLI 的最小使用闭环](<../02-开始上手/02-认识 Hermes 的基本使用方式.md>)还没跑顺，先别急着往服务化跳。
...
5. 后面的[自动化](<07-让 Hermes 自己自动跑.md>)、工作流编排、产品化接入，会更容易继续往前走
...
- 请求进来后，Hermes 仍然可以使用自己的[工具能力](../03-玩出花样/05-让工具更顺手.md)，例如终端、文件操作、网页搜索、记忆、skills 等
