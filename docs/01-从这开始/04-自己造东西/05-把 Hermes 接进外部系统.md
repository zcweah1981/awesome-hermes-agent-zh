...
- 一会儿看 MCP
- 一会儿想 Plugins
- 一会儿又想[API Server](<./06-把 Hermes 暴露成后端服务.md>)
- 一会儿又想是不是先做自己的内部扩展

但对当前这个阶段来说，最重要的不是全部都懂，而是先抓住最自然的一条主线。

这条主线通常就是：

我要让 Hermes 去用一个已经存在的外部系统 → 先看 MCP。
...
## 🎯 先把外部系统接进来以后，你真正会得到什么

这一页最值得你现在记住的，不是配置字段，而是接进来以后会发生什么变化：

1. Hermes 不再只局限在聊天窗口内部
2. 它能开始调用你已经存在的外部工具与服务
3. 你会第一次真正感受到“系统能力扩展”而不只是“功能切换”
4. 后面的[服务化](<./06-把 Hermes 暴露成后端服务.md>)、[自动化](<./07-让 Hermes 自己自动跑.md>)和 [IDE 集成](./08-放进编辑器里用.md)才开始有现实基础

一句话说透：

这一步不是让 Hermes 多一个名词，而是让 Hermes 开始真正接触外部世界。
...
- [API Server 的服务化暴露、鉴权、部署和运维](<./06-把 Hermes 暴露成后端服务.md>)
