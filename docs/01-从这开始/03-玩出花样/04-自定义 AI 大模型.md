...
这一页只解决一件事：
把一个已经会用的 Hermes，接到更适合你自己的模型路由上。
...
## 🧭 先记住：这一页不是让你一上来就折腾最复杂的模型接法

很多人一进入“玩出花样”，就会本能地去想：

- 我要不要马上接 custom endpoint
- 我要不要直接上统一网关
- 我要不要一口气把主模型、备用模型、多 key 全配好

这页要先帮你把节奏拉回来：

模型层的优化，是为了让 Hermes 更适合你，不是为了让配置本身变复杂。

所以这页真正的问题是：

你什么时候值得动模型层？动完之后，你能获得什么？
...
先回到[05-配好 AI 大模型并完成第一次互动](../01-先跑起来/05-配好 AI 大模型并完成第一次互动.md)，先用默认可行路线接通，再来做优化。

---

## 🎯 动完模型层以后，你真正会得到什么

这页不是让你“拥有更多模型名词”，而是让你拿到这些更实在的结果：

1. 日常使用更稳，不会总被单一路线卡住
2. 成本、速度、能力更贴近你的真实使用场景
3. 你知道主路、备路、网关、自托管分别该放在哪一层
4. 你以后碰到模型问题时，更容易判断到底是哪一层出了问题

一句话说透：

模型层调顺以后，Hermes 不只是“能回答”，而是更像“能长期陪你稳定工作”。
...
解决的是：
同一家 provider 的 key 不稳、容易限流、容易撞 quota。
...
如果你现在真正想做的是“让前端、客户端或应用把 Hermes 当后端来接”，那不是这一节的模型网关问题，而是要去看[06-把 Hermes 暴露成后端服务](../04-自己造东西/06-把 Hermes 暴露成后端服务.md)。
...
![终端配置演示图：在临时 HERMES_HOME 中用 hermes config set 写入 provider 与 model，随后由检查脚本读出当前配置值，并显示 auth 相关条目存在](../../assets/rm2-4-custom-llm-02-provider-config-success.png)

![终端聊天结果截图：在临时 HERMES_HOME 下执行一条非固定中文 query 后，界面返回欢迎语、两项可做事项，并显示 session_id](../../assets/rm2-4-custom-llm-03-custom-model-success.png)
