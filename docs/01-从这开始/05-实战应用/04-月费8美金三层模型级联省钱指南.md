...
![Hermes 三层模型级联总览：Default、Medium、Heavy 三档按任务复杂度路由，把月费控制在约 8 美金](../../assets/practical-v2-04-model-cascade-saving-00-routing-overview-cn.webp)

---

## 🔎 搜索收录速答

三层模型级联适合想用低成本模型处理日常任务、只在复杂推理时切到强模型的 Hermes 用户。推荐把任务分成轻量问答、代码/工具调用、复杂规划三层，再用 provider 和模型路由控制成本。继续优化时可以结合[CLI 命令参考](/docs/reference/cli-commands)、[模型配置](/docs/start/personalize/custom-model)和[Kanban 多 Agent 编排](/docs/start/practical/kanban-multi-agent)。
...
![Hermes 模型级联成本看板：通过预算护栏、任务分布和路由规则控制模型调用费用](../../assets/practical-v2-04-model-cascade-saving-01-cost-dashboard-cn.webp)
...
- 回到[实战应用总览](/docs/start/practical)，根据任务复杂度继续选择成本优化、自托管或自动化实践。
