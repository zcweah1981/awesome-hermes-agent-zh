...
![Hermes Agent 生产安全加固四层纵深防御：第一层用户白名单与 Role 鉴权 → 第二层命令审批与危险操作拦截 → 第三层 Docker/SSH 执行隔离 → 第四层密钥保护与 redact_secrets，缺一不可](../../assets/practical-v2-10-security-hardening.webp)
