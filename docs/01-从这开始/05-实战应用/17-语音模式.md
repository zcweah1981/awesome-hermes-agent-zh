# 🎙️ 17-语音模式：让 Hermes 听懂你说的话、开口回答你

> 一句话先说清楚：这一页教你打开 Hermes 的"嘴和耳朵"——CLI 里按 Ctrl+B 就能跟它说话、Telegram 里发语音消息它能听懂、Discord 里它能加进语音频道跟你群聊。三种入口，一套 STT/TTS 引擎。语音只是入口变化，Agent 内核不变；如果你想理解内核怎么工作，配合 [22-Hermes Agent 深度拆解与自建指南](./22-Hermes%20Agent%20深度拆解与自建指南.md) 一起看。

![Hermes Agent 语音模式三大入口架构：CLI Ctrl+B 本地录音、Telegram 语音消息上传、Discord 语音频道加入；统一经 STT（faster-whisper）转文字 → Agent Loop → TTS（Edge TTS / OpenAI）转语音回应](../../assets/practical-v2-11-voice-mode.webp)
