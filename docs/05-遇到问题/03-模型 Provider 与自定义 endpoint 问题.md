...
> 只要 `hermes version` 和 `hermes doctor` 还能正常跑，模型 / Provider 问题通常就不是[安装层](./02-安装更新与环境问题.md)，而是鉴权、模型名、endpoint 或兼容能力没对齐。
...
- 什么情况下优先用 Hermes 官方 provider，什么情况下才该走[08-自定义兼容接口](../03-国内落地/02-国内模型/08-自定义兼容接口.md)？
...
- 还是 custom endpoint 的[兼容层本身不完整](../03-国内落地/02-国内模型/08-自定义兼容接口.md)
...
- 如果你其实连基础命令和安装链路都还没跑顺，先回 [02-安装 / 更新 / 环境问题](./02-安装更新与环境问题.md)
...
- 如果你已经确认是套餐 / 额度 / 权限问题，转去 [02-国内模型 | 总览](../03-国内落地/02-国内模型/01-总览.md) 再进入对应厂商页
...
- 如果你其实应该走官方 provider，不要继续硬调 endpoint，回 `hermes model` 重选；如果你当前真正卡的是 CLI 里怎么切模型 / 会话，再去看 [04-CLI / TUI / 会话问题](<./04-CLI TUI 与会话问题.md>)
...
- 如果你只是 custom endpoint 模型名对不上，继续留在本页或去 [08-自定义兼容接口](../03-国内落地/02-国内模型/08-自定义兼容接口.md)
...
- 如果你只是第一次把 Hermes 跑通，不要先走 custom endpoint，先回 [02-安装 / 更新 / 环境问题](./02-安装更新与环境问题.md) 确认基础链路和官方 provider 已经稳定
...
- 如果你要的是稳定 Agent 工作流，优先回官方 provider；如果你已经开始碰到工具调用和会话行为异常，也一起去看 [06-Tools / Skills / MCP 问题](<./06-Tools Skills MCP 问题.md>) 与 [04-CLI / TUI / 会话问题](<./04-CLI TUI 与会话问题.md>)
...
- 如果你回头发现其实不是 provider 层，而是安装、PATH、shell 或环境混乱，退回 [02-安装 / 更新 / 环境问题](./02-安装更新与环境问题.md)
