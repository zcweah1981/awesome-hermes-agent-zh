...
### 第 2 步：确认你走的是 WebSocket 模式

如果你是先在本机或内网机器上跑 Hermes，最推荐的就是 WebSocket 模式。
...
如果你已经有稳定可访问的公网服务，再考虑 webhook 模式。

---

### 第 3 步：补齐最基本的飞书凭证

手工配置时，把这些写到 `~/.hermes/.env`：
...
- `FEISHU_ALLOWED_USERS`：允许使用机器人的飞书用户 Open ID 白名单
- `FEISHU_HOME_CHANNEL`：把哪一个聊天设成 Home Chat，用来收 cron 结果和跨平台通知

成功信号：
- `.env` 里已经有飞书相关配置
- `hermes gateway` 启动时能读取到这些值

如果没成功，先检查：
- App ID / Secret 有没有抄错
- `FEISHU_DOMAIN` 是不是写成了别的平台值
- 配置是否真的写进了 Hermes 当前使用的 `.env`
