# 让 Hermes 记住你：持久记忆只看 USER.md 和 MEMORY.md
...
下一步通常会进入“自定义 AI 大模型”，也就是继续把同一个 Hermes 调得更适合你的成本、速度和模型来源。

但当前仓库这页还没创建，所以这里先不放下一页链接，避免坏链。
...
- Sessions（只用于理解 session / sessions / session_search 的边界）：`https://hermes-agent.nousresearch.com/docs/user-guide/sessions`
