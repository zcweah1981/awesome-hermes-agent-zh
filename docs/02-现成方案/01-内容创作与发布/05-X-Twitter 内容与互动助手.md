...
![对比图：左侧是默认可用的读取能力，适合先搜索推文、读取上下文和监控提及；右侧是需要显式开启的写操作，包含发推、回复、点赞和私信，强调所有动作都以你的账号身份执行](../../assets/solution-twitter-read-vs-actions-v1.webp)
...
![结构图：从安装 hermes-tweet、配置 X API 凭据、先跑读取测试，到确认无误后再决定是否开启写操作的最短链路](../../assets/solution-twitter-setup-chain-v1.webp)
