...
这一页只解决一件事：
把外部 memory provider 放回正确位置，再决定你下一步该走 `全息记忆.md`、`Honcho记忆.md`，还是先去 `对比.md`。
...
[Holographic](./全息记忆.md) → [Honcho](./Honcho记忆.md) → [compare](./对比.md)
...
- 你只是第一次接外部 provider：先看 [Holographic](./全息记忆.md)
- 你已经明确是多 profile / 多助手系统：先看 [Honcho](./Honcho记忆.md)
- 你当前拿不准方向：先看 [compare](./对比.md)
...
- [让 Hermes 记住你：持久记忆只看 USER.md 和 MEMORY.md](../../玩出花样/持久记忆.md)
- [多个助手一起工作：先理解 Profiles](../多助手.md)
